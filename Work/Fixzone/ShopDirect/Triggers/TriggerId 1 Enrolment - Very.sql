...
WHERE 
  ValidFlag=1 AND 
  Customer.CUSTOMERID IS NOT NULL AND
  COALESCE(Customer.EMAIL,'')<>'' AND
  Customer.RetailClientID=2 AND
  Customer.UserID='STREAMLINE' AND
  Custapl.POLICYNUMBER like '%ESP' AND
  CustomerEnrolment.LinkType=0 AND TriggerRes.id IS NULL AND
  CustApl.CUSTOMERID IS NOT NULL 
GROUP BY
  enroleid,
  email,
  enrolecode,
