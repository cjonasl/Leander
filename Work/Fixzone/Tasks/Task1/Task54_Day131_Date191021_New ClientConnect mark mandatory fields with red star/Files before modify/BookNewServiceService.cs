...
namespace ClientConnect.Services
{
    public class BookNewServiceService : Service, IService
    {
        ...
        public Registration_SendModelResult Generate3CModel()
        {
            ...
            //customer info
            if (!string.IsNullOrEmpty(customer.MobileTel)) customer.MobileTel = customer.MobileTel.Trim();
            if (!string.IsNullOrEmpty(customer.LandlineTel)) customer.LandlineTel = customer.LandlineTel.Trim();
            ...
            result.unitInfo.customerInfo.fixedPhone = string.IsNullOrEmpty(customer.LandlineTel) ? string.Empty : phonePrefix + customer.LandlineTel.TrimStart('0');
            result.unitInfo.customerInfo.lastName = customer.Surname;
            result.unitInfo.customerInfo.mobilePhone = string.IsNullOrEmpty(customer.MobileTel) ? string.Empty : phonePrefix + customer.MobileTel.TrimStart('0');
