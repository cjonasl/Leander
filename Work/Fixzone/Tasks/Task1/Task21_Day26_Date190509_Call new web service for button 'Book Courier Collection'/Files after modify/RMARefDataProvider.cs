...
namespace Mobile.Portal.DAL
{
    ...
    public class RMARefDataProvider : DataAccess<RMARef>, Mobile.Portal.DAL.IRMARefDataProvider
    {
        ...
        public void UpdateCollectionjob(string RMA, string jobref, string collectionDate, bool SwapclaimCollection, int shipmateConsignmentCreationId = 0)
        {
            ...
            if (shipmateConsignmentCreationId != 0)
                parameters = new SqlParameter[5];
            else
                parameters = new SqlParameter[4];

            parameters[0] = new SqlParameter("@RMA", RMA);
            parameters[1] = new SqlParameter("@jobref", jobref);

            if (collectionDate != null)
                parameters[2] = new SqlParameter("@collectionDate", collectionDate);
            else
                parameters[2] = new SqlParameter("@collectionDate", DBNull.Value);

            parameters[3] = new SqlParameter("@SwapclaimCollection", SwapclaimCollection);

            if (shipmateConsignmentCreationId != 0)
                parameters[4] = new SqlParameter("@ShipmateConsignmentCreationId", shipmateConsignmentCreationId);
