...
R.ClientRef,
		...
		R.ResMediaURL AS ShipmateMediaURL


    FROM SAEDIParts P WITH (nolock)

    LEFT OUTER JOIN (SELECT DISTINCT RMA.SaediFromID, RMA.ClientRef, RMA.RmaId , RMA.INPUT_externalMatId, RMA.INPUT_partNumberReceived, RMA.RmaDocumentUrl, RMA.ErrorMessage, RMA.Success, RMA.collectionaddedon, RMA.CollectionDate,
     RMA.Collectionref, SCC.ResMediaURL

    FROM SONYRMA RMA WITH (nolock)
	LEFT JOIN ShipmateConsignmentCreation SCC WITH (nolock) ON RMA.ShipmateConsignmentCreationId = SCC.ID
