    ...
    public partial class PartStock : System.Web.UI.Page
    {
        ...
        protected void partsOrderGridView_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "BookCourier")
            {
                ...
                string queryString = shipmate.BtnBookCourierClick(session.Login.SaediId, rmaId);
                ...
                foreach( RMARef item in result)
                {
                    ...
                    if (!string.IsNullOrEmpty(item.Collectionref))
                    {
                        ...
                        else //New Shipmate (CollectionDate is not set)
                        {
                            ...
                            returnCourierResult.AppendFormat(" <a href='{0}{1}' target='_blank'>Track</a> <br/>", System.Configuration.ConfigurationManager.AppSettings["TNTTrackAndTraceUrl"], item.Collectionref);
                            returnCourierResult.AppendFormat(" <a href='ShowShipmateLabel.aspx?TrackingReference={0}' target='_blank'>Show courier label</a> <br/>", item.Collectionref);
                        }
