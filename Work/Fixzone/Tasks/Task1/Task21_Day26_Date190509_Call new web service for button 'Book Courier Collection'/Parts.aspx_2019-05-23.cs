...
namespace MobilePortal
{
    public partial class PartsPage : System.Web.UI.Page
    {
        ...
        protected void BtnBookCourier_Click_Tmp(object sender, CommandEventArgs e)
        {
        ...
        protected void BtnBookCourier_Click(object sender, CommandEventArgs e)
        {
            ...
            toAddress = new ToAddressRequest(name, addressLine, city, postcode, country);

            parcel = new Parcel(rmaId + "-1", 3000, 20, 10, 15);
            parcels.Add(parcel);

            consignmentRequest = new ConsignmentRequest(Convert.ToInt32(call.Id), 123, rmaId, fzShipMate.Token, "DPDNEXT", toAddress, parcels);

            try
            {
                consignmentResponse = fzShipMate.CreateConsignment(consignmentRequest);
            }
            catch (Exception ex)
            {
                onlineBookingLogBLL.InsertOnlineServiceLog(consignmentRequest, ex.Message + " " + consignmentResponse, call.SaediFromId, fzShipMate.GetUrl(FzShipMateAction.CreateConsignment), false);
                throw;
            }

            if (consignmentResponse.message == "Consignment Created")
            {
                rmaRefBLL.UpdateCollectionjob(rmaId, consignmentResponse.data[0].tracking_reference, null, false);
                onlineBookingLogBLL.InsertOnlineServiceLog(consignmentRequest, consignmentResponse, call.SaediFromId, fzShipMate.GetUrl(FzShipMateAction.CreateConsignment), true);
                ShowConsignmentDetails(consignmentResponse.data[0].tracking_reference, "Consignment was created successfully");
            }
            else
            {
                onlineBookingLogBLL.InsertOnlineServiceLog(consignmentRequest, consignmentResponse.message + " " + consignmentResponse, call.SaediFromId, fzShipMate.GetUrl(FzShipMateAction.CreateConsignment), false);
        ...
        private void ShowConsignmentDetails(string trackingReference, string title)
        {
            FzShipMate fzShipMate;
            ConsignmentResponse consignmentResponse;
            string queryString;

            fzShipMate = new FzShipMate();
            consignmentResponse = fzShipMate.GetLabel(trackingReference);

            queryString = string.Format("ShipMate.aspx?" +
                "ShipMateTitle={0}&" +
                "ShipMateConsignmentReference={1}&" +
                "ShipMateParcelReference={2}&" +
                "ShipMateCarrier={3}&" +
                "ShipMateServiceName={4}&" +
                "ShipMateTrackingReference={5}&" +
                "ShipMateCreatedBy={6}&" +
                "ShipMateCreatedWith={7}&" +
                "ShipMateCreatedAt={8}&" +
                "ShipMateDeliveryName={9}&" +
                "ShipMateLine={10}&" +
                "ShipMateCity={11}&" +
                "ShipMatePostcode={12}&" +
                "ShipMateCountry={13}",
                title,
                consignmentResponse.data[0].consignment_reference,
                consignmentResponse.data[0].parcel_reference,
                consignmentResponse.data[0].carrier,
                consignmentResponse.data[0].service_name,
                consignmentResponse.data[0].tracking_reference,
                consignmentResponse.data[0].created_by,
                consignmentResponse.data[0].created_with,
                consignmentResponse.data[0].created_at,
                consignmentResponse.data[0].to_address.delivery_name,
                consignmentResponse.data[0].to_address.line_1,
                consignmentResponse.data[0].to_address.city,
                consignmentResponse.data[0].to_address.postcode,
                consignmentResponse.data[0].to_address.country );
        ...
        {
            ShowConsignmentDetails(e.CommandArgument.ToString(), "Consignment details");
        }
    }
}
