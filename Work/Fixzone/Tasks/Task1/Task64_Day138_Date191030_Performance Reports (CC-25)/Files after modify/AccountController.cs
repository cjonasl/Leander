    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult SignIn(AccountViewModel model)
        {
            ...
            if (ValidRegex.IsMatch(model.UserId))
            {
                ...
                if (accDetails != null)
                {
                    if (accDetails.IsCallCenterUser || accDetails.UserStoreId == 0)
                    {
                        ModelState.AddModelError("Password", "Your account has not been set as client user.");
                        return View(model);
                    }
                ...
                ClientBusinessRule.AddBusinessRulesInSession(accDetails.UserStoreId, Session);
                accService.ClearFromSession();
                accService.SessionInfo.UserId = model.UserId;
                accService.SessionInfo.ClientId = accDetails.UserStoreId;
                accService.SessionInfo.IsAdm = accDetails.UserLevel == 1 ? true : false;
                accService.SessionInfo.IsSuperAdm = accDetails.UserLevel == 0 ? true : false;
                accService.SessionInfo.ShowReports = (Session[BusinessRuleKey.ShowReports.ToString()] == null || ((string)Session[BusinessRuleKey.ShowReports.ToString()]) == "true") ? true : false;

                var storedInfo = accService.CookiesStoredInfo;

                if (storedInfo != null && storedInfo.StoreId != accDetails.UserStoreId.ToString())
                {
                ...
                {
                    accService.SessionInfo.UserId = model.UserId;
                    return Redirect(Url.Process(PredefinedProcess.ClientExpiredPassword));
                }
        ...
        public ActionResult SignOut()
        {
            ...
            if (Settings.Default.DeploymentTarget == "Clients")
                ...
                return Redirect("~/CallCenter/SignIn");
        }
