...
namespace CAST.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult SignIn(string returnUrl)
        {
            ViewBag.HideBack = true;
            ViewBag.HideHome = true;
            ViewBag.CustomerUrnDisplayValue = "none";
            ViewBag.ClientName = "";
        ...
        {
            CustomerAccount accdetail = null;
            List<CustomerAccount> accDetails = null;

            ViewBag.HideBack = true;
            ViewBag.HideHome = true;
            ViewBag.CustomerUrnDisplayValue = "none"; //Default
            ...
            else
            {
                ...
                ViewBag.ClientName = model.Email;
            }
            ...
            if (!string.IsNullOrEmpty(model.Email))
            {
                accDetails = accService.GetAccountDetailsByEmail(model.Email, int.Parse(RetailClient_ID));

                if (accDetails.Count > 1)
                {
                    ViewBag.Site = System.Configuration.ConfigurationManager.AppSettings["Brand"].ToString();
                    ViewBag.Domain = System.Configuration.ConfigurationManager.AppSettings["client_domain"].ToString();
                    ViewBag.CustomerUrnDisplayValue = "block";
                }
            }
            else
            {
            ...
            if (accDetails.Count > 1 && string.IsNullOrEmpty(model.ClientCustRef) && !string.IsNullOrEmpty(model.Email) && !string.IsNullOrEmpty(model.ClientName) && (model.Email == model.ClientName))
            {
                ModelState.AddModelError("ClientCustRef", "Please enter account number.");
            }
            else if (!string.IsNullOrEmpty(model.ClientCustRef))
            {
                customerAccount.ClientCustRef = model.ClientCustRef;
            }
            ...
            {
                ModelState.AddModelError("Password", "The log in details do not match our records, please try again.");
                return View(new CustomerAccount());
            }
            else if (accDetails.Count == 1)
            {
                accdetail = accDetails.FirstOrDefault();
                valid = BCryptHelper.CheckPassword(model.Password.Trim(), accdetail.Password);
            }
            else if (accDetails.Count > 1 && string.IsNullOrEmpty(model.ClientCustRef))
            {
                return View(customerAccount);
            }
            else
            {
                accdetail = accDetails.Where(x => x.ClientCustRef.ToLower().Trim() == model.ClientCustRef.ToLower().Trim() && BCryptHelper.CheckPassword(model.Password.Trim(), x.Password)).FirstOrDefault();
                ...
                {
                    accdetail = accDetails.First(); //Need the email to update try count below
                }
            }
