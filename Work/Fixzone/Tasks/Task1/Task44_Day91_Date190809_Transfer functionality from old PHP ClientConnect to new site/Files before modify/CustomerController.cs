...
namespace ClientConnect.Controllers
{
    ...
    public class CustomerController : Controller
    {
        ...
        public ActionResult ServiceList(int? custaplid)
        {
            List<JobDetailsModel> services = new List<JobDetailsModel>();
        ...
        public ActionResult Search(int? page)
        {
            ...
            if (model.TotalRecords == 1)
            {
                ...
                if (customerService.IsBackButtonPressed) return View(model);
       ...
       // public ActionResult Details(int? Customerid)
        ...
        public ActionResult FindCustomer(string customerSearchCriteria)
        {

            customerService.SessionInfo.SearchCriteria = customerSearchCriteria.Trim();
            customerService.SessionInfo.PageNumber = 1;
            return Redirect(Url.Process(PredefinedProcess.CustSearch));

            
        //    GetCustomerInfo.SessionInfo.SearchCriteria = productSearchCriteria;
        //    ProductService.SessionInfo.PageNumber = 1;
        //    return Redirect(Url.Process(PredefinedProcess.ProductSearch));
        }
        public ActionResult AdvCustomerSearch()
        {
            return Redirect(Url.Process(PredefinedProcess.CustAdvSearch));
        }
        public ActionResult AdvSearch(int? page)
        {
            if (page == null) page = customerService.SessionInfo.PageNumber == 0 ? 1 : customerService.SessionInfo.PageNumber;
            customerService.SessionInfo.PageNumber = page ?? 1;
            Customer_SearchModel modelsession = new Customer_SearchModel();
            AdvSearchCriteria AdvSearchCriteria = new AdvSearchCriteria();
            AdvSearchCriteria.Postcode = customerService.SessionInfo.Postcode;
            AdvSearchCriteria.UseAndInWhereCondition = true;
            AdvSearchCriteria.Surname = customerService.SessionInfo.Surname;
            AdvSearchCriteria.PolicyNumber = customerService.SessionInfo.PolicyNumber;
            AdvSearchCriteria.ClientCustRef = customerService.SessionInfo.ClientCustRef;
            AdvSearchCriteria.Address = customerService.SessionInfo.Address;
            AdvSearchCriteria.TelNo = customerService.SessionInfo.TelNo;
            ...
            if (model.TotalRecords == 1)
            {
                // Checking if user push back button on ShowDetails
                if (customerService.IsBackButtonPressed) return View(model);
                return RedirectToAction("GoToDetails", new { id =model.SearchResults[0].CustomerId });
            }
            ViewBag.OnePageOfJobs = new StaticPagedList<Customer_SearchResult>(model.SearchResults, customerService.SessionInfo.PageNumber, Settings.Default.JobSearchPageSize, model.TotalRecords);
        ...
        public ActionResult AdvSearchCriteria(AdvSearchCriteria model, int? page)
        {
          ...
          //  JobService.SessionInfo.SearchJobId = string.IsNullOrEmpty(model.JobId) ? string.Empty : model.JobId;
            ...
            if (result.TotalRecords == 1)
            {
                // Checking if user push back button on ShowDetails
                if (customerService.IsBackButtonPressed) return View(model);
                return RedirectToAction("GoToDetails", new { id =result.SearchResults[0].CustomerId });
            }
