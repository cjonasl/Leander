--exec dbo.Report_GetClosedJobs 818, '2019-11-01', '2019-11-30'

--exec dbo.Report_GetClosedJobsBetweenTwoDates

ALTER PROCEDURE [dbo].[Report_GetClosedJobsBetweenTwoDates]
@ClientId int,
@StartDate date,
@EndDate date,
@IsForExport bit = NULL
AS
BEGIN
...
BEGIN
  SELECT
    ...
    dbo.fn_Help1GetValue(cap.[APPLIANCECD]) AS [Appliance category],
    dbo.fn_Help1GetValue(cap.[MFR]) AS Brand,
    dbo.fn_Help1GetValue(cap.[MODEL]) AS Model,
    dbo.fn_Help2GetValue(a.[FirstDateoffered]) AS [First date offered],
    dbo.fn_Help2GetValue(a.[DateChosen]) AS [Customer selected date],
    dbo.fn_Help2GetValue(d2.[EventDate]) AS [Date of first visit],
    dbo.fn_Help2GetValue(d4.[EventDate]) AS [Date of second visit],
    dbo.fn_Help2GetValue(d6.[EventDate]) AS [Date of third visit],
    dbo.fn_Help1GetValue(s.[Status]) AS [Job status],
    dbo.fn_Help2GetValue(s1.CLOSEDDATE) AS [Closed date],
    dbo.fn_Help1GetValue(cap.[SNO]) AS [Serial number],
    dbo.fn_Help1GetValue(s1.[CLIENTREF]) AS [Client Reference Number],
    dbo.fn_Help1GetValue(cap.[POLICYNUMBER]) AS [Policy number]
  FROM 
    @TmpTableService3 s1
