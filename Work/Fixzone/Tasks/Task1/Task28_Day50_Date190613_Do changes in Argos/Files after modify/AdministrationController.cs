...
namespace CAST.Controllers
{
    ...
    public class AdministrationController : DataController
    {
        ...
        public ActionResult UserInformationNewUser(string id)
        {
            _adminService.SetUserIdForEditing(id);
            var model = _adminService.GetUserDetailsForUpdate(id);
            model.LevelsOfAccess = _userService.GetUserLevelsOfAccess();
            ViewBag.newUserSavedMsg = "<div class=\"success\" style=\"text-align: center !important;\">The user was successfully created</div>";
        ...
        public ActionResult AddColleague(AdministrationUserModel model)
        {
            ...
            if (isUserSaved)
            {
                return RedirectToAction("UserInformationNewUser", new { id = model.UserId });
            }
            else
            {
                ViewBag.IsCallCenter = _storeService.IsCallCenter();
