...
namespace CAST.Controllers
{
    ...
    public class UserController : DataController
    {
        ...
        public ActionResult SignIn(User_DetailsModel model)
        {
            ...
            if (userInfo.UserLevel == 0) //SuperAdm
            ...
            if (!string.IsNullOrEmpty(userInfo.UserId))
            {
                ...
                if (userInfo.UserLevel == 0) //SuperAdm
                {
                    RemoveCookie("CAST_Store");
                    HttpContext.Session["IsSuperAdm"] = 1;

                    return Redirect(Url.Process(PredefinedProcess.ClientSearch));
                }
                else
                {
                    if (HttpContext.Session["IsSuperAdm"] != null)
                        HttpContext.Session.Remove("IsSuperAdm");
                }
        ...
        public JsonResult DisplayConfirmMessage(string user, string password)
        {
            ...
            if (!string.IsNullOrEmpty(user) && !string.IsNullOrEmpty(password))
            {
                ...
                if (model.UserCanLogIn && !model.IsSuperAdm) //Do not need to show this store info for super administrator who is able to switch between different stores
                {
        ...
        public ActionResult ShowConfidentialDetails(User_ConfidentialInfoModel model)
        {
            ...
            if(ModelState.IsValid)
            {
                model.UserId = _userService.GetFirstTimeUserId();
                _userService.SaveUserDetails(model);
                var info = _userService.GetUserInformation(model.UserId);
        ...
        public ActionResult SearchNewClient()
        {
            RemoveCookie("CAST_Store");
            return Redirect(Url.Process(PredefinedProcess.ClientSearch));
        }
        ...
        {
            CAST.Models.Store.StoreInfoModel storeInfoModel = _storeService.GetStoreInfo(id);
            _userService.SetStore(id, storeInfoModel.StoreName);
