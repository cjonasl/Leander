...
namespace ClientConnect.Controllers
{
    ...
    public class BookNewServiceController : Controller
    {
        ...
        public bool AddressIsEditable { get; set; }
        public bool ContactIsEditable { get; set; }
        ...
        public BookNewServiceController()
        {
            ...
            try
            {
                BlockSupplierList = BusinessRules.Where(x => x.Key == BusinessRuleKey.BlockSupplierList.ToString()).FirstOrDefault().Checked;
            }
            catch
            {
                BlockSupplierList = false;
            }
            try
            {
            ...
            try
            {
                ShowAccountNumber = BusinessRules.Where(x => x.Key == BusinessRuleKey.ShowAccountNumber.ToString()).FirstOrDefault().Checked;
            }
            catch
            {
            ...
            //Configuration editable fields in CustomerPreviewPage
            CustomerIsEditable = BusinessRule.GetValue(BusinessRules, BusinessRuleKey.CustomerPreviewPage_Customer_Is_Editable);
            AddressIsEditable = BusinessRule.GetValue(BusinessRules, BusinessRuleKey.CustomerPreviewPage_Address_Is_Editable);
            ContactIsEditable = BusinessRule.GetValue(BusinessRules, BusinessRuleKey.CustomerPreviewPage_Contact_Is_Editable);
        }
        ...
        private void GetDropDownListVisitcodesSelectedValue(out string selectedValue, out bool enabled)
        {
            ...
            //For clients visit code should always be: "New Service Call" (000)
            if (Settings.Default.DeploymentTarget == "Clients")
            {
                selectedValue = "000";
                enabled = false;
            }
        ...
        public ActionResult CustomerPage()
        {
            ...
            model.RetailClientList = CustomerService.GetRetailClientList();
            model.CountryList = CustomerService.GetCountryList(model.Country);
            //model.SupplierList = CustomerService.GetSupplierList(BookService.SessionInfo.);
            model.ContactMethodList = CustomerService.GetContactMethodList(model.ContactMethod.ToString());
            model.TitleList = CustomerService.GetTitlesList(model.Title);
            ViewBag.CustomerLogged = AdminService.StoreId == 0;

            ViewBag.RetailClientIsMandatory = CustomerValidation.RetailClientIsMandatory;
            ViewBag.CLIENTCUSTREFIsMandatory = CustomerValidation.CLIENTCUSTREFIsMandatory;
            ViewBag.TitleIsMandatory = CustomerValidation.TitleIsMandatory;
            ViewBag.ForenameIsMandatory = CustomerValidation.ForenameIsMandatory;
            ViewBag.SurnameIsMandatory = CustomerValidation.SurnameIsMandatory;
            ViewBag.PostcodeIsMandatory = CustomerValidation.PostcodeIsMandatory;
            ViewBag.Addr1IsMandatory = CustomerValidation.Addr1IsMandatory;
            ViewBag.Addr2IsMandatory = CustomerValidation.Addr2IsMandatory;
            ViewBag.Addr3IsMandatory = CustomerValidation.Addr3IsMandatory;
            ViewBag.TownIsMandatory = CustomerValidation.TownIsMandatory;
            ViewBag.CountyIsMandatory = CustomerValidation.CountyIsMandatory;
            ViewBag.CountryIsMandatory = CustomerValidation.CountryIsMandatory;
            ViewBag.Tel1IsMandatory = CustomerValidation.Tel1IsMandatory;
            ViewBag.Tel2IsMandatory = CustomerValidation.Tel2IsMandatory;
            ViewBag.EmailIsMandatory = CustomerValidation.EmailIsMandatory;
            ViewBag.ContactMethodIsMandatory = CustomerValidation.ContactMethodIsMandatory;

            //Set default values
            if (ContactIsEditable)
            {
            ...
            ViewBag.ShowStarMandatoryField = CustomerValidation.RetailClientIsMandatory || CustomerValidation.CLIENTCUSTREFIsMandatory || CustomerValidation.TitleIsMandatory || CustomerValidation.ForenameIsMandatory || CustomerValidation.SurnameIsMandatory || CustomerValidation.PostcodeIsMandatory || CustomerValidation.Addr1IsMandatory || CustomerValidation.Addr2IsMandatory || CustomerValidation.Addr3IsMandatory || CustomerValidation.TownIsMandatory || CustomerValidation.CountyIsMandatory || CustomerValidation.CountryIsMandatory || CustomerValidation.Tel1IsMandatory || CustomerValidation.Tel2IsMandatory || CustomerValidation.EmailIsMandatory || CustomerValidation.ContactMethodIsMandatory;
        ...
        public ActionResult CustomerPage(CustomerPageModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (BookService.SessionInfo.OnlinebookingFailed)
                {
                    ...
                    ViewBag.ErrorMessage = "Problem on connecting to onlinebooking . Switched to Offline booking mode";
                    model.CustomerId = BookService.SessionInfo.CustomerId;
                    tempCustomerId = BookService.CreateCustomerBackup(model,BookService.SessionInfo.OnlinebookingFailedReason);
                    model.CustomerId = tempCustomerId;
                    BookService.SessionInfo.CustomerId = tempCustomerId;
                    return Redirect(Url.ProcessNextStep());
                }
        ...
        {
            ViewBag.ProductNumberCodeIsMandatory = BookService.SessionInfo.ProductNumberCodeIsMandatory = false; //Default
            ...
            int modelId = custProd.ModelId > 0 ? custProd.ModelId : ProductService.SessionInfo.ModelId;

            if (modelId > 0)
            {
                ViewBag.ProductNumberCodeIsMandatory = BookService.SessionInfo.ProductNumberCodeIsMandatory = ProductService.ProductNumberCodeIsMandatory(modelId);
            }
            ...
            custProd.RetailerNameList = JobService.GetRetailerList(custProd.RetailerId.ToString());
           // custProd.RetailerId = JobService.GetRetailername(custProd.RetailerId);
            // custProd.ClientRef = BookService.SessionInfo.CustProd.CustAplId;
            ViewBag.ShowOrderNumber = showOrderNumber;
            ViewBag.BlockSupplierList = BlockSupplierList;
        ...
        public ActionResult ApplianceDetails(CustomerProductModel model, bool novalidation = false)
        {
            ...
            ViewBag.ProductNumberCodeIsMandatory = BookService.SessionInfo.ProductNumberCodeIsMandatory;
            ViewBag.NOShowWarrantyInfo = NOApplianceWarrantyInfo;
            ViewBag.ClientBookingType = storeService.ClientBookingType;
            ViewBag.ShowOrderNumber = showOrderNumber;
            ViewBag.ShowRegno = ShowRegno;
            ...
            if (BookService.SessionInfo.ProductNumberCodeIsMandatory && string.IsNullOrEmpty(model.PNC))
            {
                ModelState.AddModelError("PNC", "This field is required. PNC (Product Number Code) can be found on the serial plate of the product.");
                return View(model);
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                Log.File.Info("Input :" + JsonConvert.SerializeObject(model));
                if (model.ModelId == 0)
                {

                    model.ModelNumber = ProductService.SessionInfo.ModelNumber;
                    model.ItemCode = ProductService.SessionInfo.ItemCode;
                    model.ApplianceCD = ProductService.SessionInfo.ApplianceCD;
                    model.ManufactName = ProductService.SessionInfo.ManufactName;
                    model.MFR = ProductService.SessionInfo.MFR;
                ...
                result.Supplierid = model.RetailerId;
                JobService.UpdateSupplierID(model.RetailerId, result.CustAplID, BookService.SessionInfo.OnlinebookingFailed);
                Log.File.InfoFormat("{0}",BookService.Msg.GenerateLogMsg("Output :" + JsonConvert.SerializeObject(result)));
        ...
        public ActionResult CustomerPreviewPage()
        {
            ...
            //Category Custonmer
            ViewBag.TitleIsEditable = CustomerIsEditable;
            ViewBag.ForenameIsEditable = CustomerIsEditable;
            ViewBag.SurnameIsEditable = CustomerIsEditable;

            //Category Address
            ViewBag.PostcodeIsEditable = AddressIsEditable;
            ViewBag.Addr1IsEditable = AddressIsEditable;
            ViewBag.Addr2IsEditable = AddressIsEditable;
            ViewBag.Addr3IsEditable = AddressIsEditable;
            ViewBag.TownIsEditable = AddressIsEditable;
            ViewBag.CountyIsEditable = AddressIsEditable;
            ViewBag.CountryIsEditable = AddressIsEditable;

            //Category Contact
            ViewBag.Tel1IsEditable = ContactIsEditable;
            ViewBag.Tel2IsEditable = ContactIsEditable;
            ViewBag.EmailIsEditable = ContactIsEditable;
            ViewBag.ContactMethodIsEditable = ContactIsEditable;
            ...
            ViewBag.ShowStarMandatoryField = (CustomerValidation.TitleIsMandatory && CustomerIsEditable) || (CustomerValidation.ForenameIsMandatory && CustomerIsEditable) || (CustomerValidation.SurnameIsMandatory && CustomerIsEditable) || (CustomerValidation.PostcodeIsMandatory && AddressIsEditable) || (CustomerValidation.Addr1IsMandatory && AddressIsEditable) || (CustomerValidation.Addr2IsMandatory && AddressIsEditable) || (CustomerValidation.Addr3IsMandatory && AddressIsEditable) || (CustomerValidation.TownIsMandatory && AddressIsEditable) || (CustomerValidation.CountyIsMandatory && AddressIsEditable) || (CustomerValidation.CountryIsMandatory && AddressIsEditable) || (CustomerValidation.Tel1IsMandatory && ContactIsEditable) || (CustomerValidation.Tel2IsMandatory && ContactIsEditable) || (CustomerValidation.EmailIsMandatory && ContactIsEditable) || (CustomerValidation.ContactMethodIsMandatory && ContactIsEditable);
        ...
        {
            string notes;
            bool modelHasChanged, postCodeHasChanged, adr1HasChanged, townHasChanged;
            ...
            if (ModelState.IsValid)
            {
                ...
                modelHasChanged = model.ModelHasChanged(CustomerService.SessionInfo.Applianceaddress, CustomerService.GetContactMethodList(model.ContactMethod.ToString()), out postCodeHasChanged, out adr1HasChanged, out townHasChanged, out notes);

                if (modelHasChanged)
                {
                    if (postCodeHasChanged)
                    {
                        CustomerService.SessionInfo.Applianceaddress.Postcode = model.Postcode;
                    }

                    if (adr1HasChanged)
                    {
                        CustomerService.SessionInfo.Applianceaddress.Addr1 = model.Addr1;
                    }

                    if (townHasChanged)
                    {
                        CustomerService.SessionInfo.Applianceaddress.Town = model.Town;
                    }

                    BookService.CreateCustomer(model, notes: notes, customerId: model.CustomerId);
                    Session["BookServiceNotes"] = notes;
                ...
                {
                    if (Session["BookServiceNotes"] != null)
                    {
                        Session.Remove("BookServiceNotes");
                    }
                }
        ...
        public ActionResult SaveSoftService(int TroubleshootQuestionID, int TroubleshootFaultID, bool issueFixed)
        {
            ...
            //select p;
           // model.troubleshootCategory
            string TroubleShootText = softservice.OfType<TroubleshootDetail>().FirstOrDefault().TroubleshoorText;
            string softserviceHeading = model.TroubleshootFaultlist.Where(x=>x.TroubleshootFaultID== TroubleshootFaultID).FirstOrDefault().troubleshootFault;//.Where(y=>y.
            ...
            {
                BookService.SessionInfo.appointmentModel.TroubleShootDescr = "Customer tried the following trouble shoot : " + softserviceHeading + " -"+ softserviceTitle + '.';
                BookService.SessionInfo.FaultDescr = string.Format("{0} {1}Trouble shoot details :{2}", BookService.SessionInfo.FaultDescr,!string.IsNullOrEmpty(BookService.SessionInfo.FaultDescr)?",":"", BookService.SessionInfo.appointmentModel.TroubleShootDescr);
                return Redirect(Url.ProcessNextStep());
            }
        ...
        public ActionResult RequestDate()
        {
            ...
            ViewBag.GetUnitSuccess = "";
            ViewBag.GetUnitError = "";
            ViewBag.ErrorMessage = "";
            ViewBag.Replacement = null;
            ViewBag.Clientbookingdelaydays = 2;
            ...
            appointment.availabiltyResponseModel.ShowHeading = Settings.Default.DeploymentTarget == "Clients" ? false : true;

            string selectedValue;
            bool dropDownListVisitcodesEnabled;
            GetDropDownListVisitcodesSelectedValue(out selectedValue, out dropDownListVisitcodesEnabled);
            ViewBag.DropDownListVisitcodesEnabled = dropDownListVisitcodesEnabled;
            appointment.Visitcodes = JobService.GetJobTypesList(selectedValue).Where(x => !ExcludeVisitType.Any(y => y == x.Value)).ToList(); 
                   
            if (Settings.Default.DeploymentTarget == "Clients" && Request.HttpMethod.ToUpper() == "GET")
            {
                ViewBag.ShowView_RequestDate = false;
                appointment.PreferredVisitDate = null;
                return View("RequestDate", appointment);
            }
            else
                ViewBag.ShowView_RequestDate = true;
        ...
        public ActionResult BookNow(string EventDate, int Engineerid, string faultdesc, string reason = "", JobType type = JobType.Defaulttype)
        {
            ...
            model.ReportFault = string.IsNullOrEmpty(BookService.SessionInfo.appointmentModel.TroubleShootDescr) ? BookService.SessionInfo.FaultDescr : BookService.SessionInfo.FaultDescr+
                "Trouble shoot tried "+
                BookService.SessionInfo.appointmentModel.TroubleShootDescr ;
            model.SNO = BookService.SessionInfo.CustProd.SerialNumber;
            model.PolicyNumber = BookService.SessionInfo.CustProd.PolicyNumber;
            model.AuthNo = BookService.SessionInfo.CustProd.AuthNo;
            model.ClientID = storeService.StoreId;
            ...
            if (!BookService.SessionInfo.OnlinebookingFailed)
            {
                ...
                if (response.BookSuccessfully)
                {
                    ...
                    {
                        serviceNotes += string.Format("Replacement has been requested as the Model {0} is not repairable. Booked from Client connect", model.Model);
                     //   onlineBookingService.AddNote(serviceid, "Customer", string.Format("Replacement has been requested as the Model {0} is not repairable. ", model.Model));
                        ...
                        serviceNotes += string.Format("Collection job is booked from Client connect.  {0} ", BookService.SessionInfo.ServiceNotes);
                       // onlineBookingService.AddNote(serviceid, "Customer", "Collection job is booked. " + BookService.SessionInfo.ServiceNotes);
                        ...
                        serviceNotes += string.Format("PartsOnly job is booked from Client connect. {0}" + BookService.SessionInfo.PartOrderEnquiry);
                    ...
                    if (Session["FirstDayOffered"] != null)
                    {
                        if (Settings.Default.DeploymentTarget == "Clients")
                        {
                            string visitDate, preferredVisitDate;

                            visitDate = model.VisitDate.ToString("dd/MM/yyyy");  
                            preferredVisitDate =  ((DateTime)Session["PreferredVisitDate"]).ToString("dd/MM/yyyy");

                            if (visitDate == preferredVisitDate)
                            {
                                serviceNotes += string.Format("Client Preferred and Selected Date: {0}.", visitDate);
                            }                    
                            else    
                            {
                                string firstDayOffered = ((DateTime)Session["FirstDayOffered"]).ToString("dd/MM/yyyy");
                                serviceNotes += string.Format("Client Preferred Date: {0}, 1st Date Available: {1}, Client Selected Date: {2}.", preferredVisitDate, firstDayOffered, visitDate); //CC-21
                            }

                            if (Session["BookServiceNotes"] != null)
                            {
                                serviceNotes += (" When the job was booked customer data was updated: " + Session["BookServiceNotes"]);
                                Session.Remove("BookServiceNotes");
                            }
                        }
                        else
                            serviceNotes += "FirstDayOffered -" + Session["FirstDayOffered"].ToString() + ". But Customer preferred for" + model.VisitDate.ToString();           //(DateTime)Session["FirstDayOffered"], model.VisitDate);//
                    }
            ...
            else
            {
                ...
                serviceModel.VisitCode = model.VisitCode;
                serviceModel.Clientref = model.ClientRef;
                int serviceId = onlineBookingService.BookBackupJob(serviceModel, BookService.SessionInfo.CustAplCreationFailed, BookService.SessionInfo.CustomerCreationFailed, model.PolicyNumber,BookService.SessionInfo.OnlinebookingFailedReason);
                if (SubmitTemplate)
                {
                    var templateData = tempService.GetSpecificTemplateFieldsbyCstaplid(serviceModel.CustAplID);
                    onlineBookingService.AddInspection(templateData, serviceid, serviceModel.CustAplID);
        ...
        {        
            appointment.availabiltyResponseModel.FirstBookingOptionResult = null; //Default
            ViewBag.ShowAppointmentreason = Settings.Default.DeploymentTarget == "Clients" ? false : true;
            ViewBag.ShowAdditionalSlot = Settings.Default.DeploymentTarget == "Clients" ? false : true;
            appointment.availabiltyResponseModel.ShowHeading = Settings.Default.DeploymentTarget == "Clients" ? false : true;
            ...
            if (Settings.Default.DeploymentTarget == "Clients")
            {
                ...
                {
                    ViewBag.ShowView_RequestDate = false;
                    ViewBag.GetUnitError = "Please select a date";
                    return View("RequestDate", appointment);
                }
                else
                {
                    Session["PreferredVisitDate"] = appointment.PreferredVisitDate;
                }
            }
                ...
                if (!BookService.SessionInfo.OnlinebookingFailed)
                {
                    ...
                    if (response.RequestSuccess)
                    {
                        ...
                        {  // else(model.PreferredVisitDate
                            ...
                            if (Settings.Default.DeploymentTarget == "Clients")
                                ViewBag.GetUnitSuccess = "Sorry that date is not available, please select from one of the available dates below";
                            else
                                ViewBag.GetUnitSuccess = "Engineer is not available for the date chosen .Please choose one from following.";
                        ...
                        if (IsappointmentAvailable && Settings.Default.DeploymentTarget == "Clients" && response.BookingOptionResult.Count > 0)
                            response.FirstBookingOptionResult = response.BookingOptionResult.OrderBy(x => x.EventDate).First();
            ...
            if (ModelState.IsValid)
            {
                ...
                if (!BookService.SessionInfo.OnlinebookingFailed)
                {
                    ...
                    else
                    {
                        ...
                        model.availabiltyModel = request;
                        model.availabiltyResponseModel = response;
                        model.Visitcodes = JobService.GetJobTypesList(string.Empty).Where(x => !ExcludeVisitType.Any(y => y == x.Value)).ToList();
        ...
        public ActionResult GoToDetails(int modelId)
        {
             ProductService.SessionInfo.ModelId = modelId;
           //// ProductService.SessionInfo. = BookService.SessionInfo.CustProd.CustomerId; 
           //// ProductService.SessionInfo.AdditionalJob = AdditionalJob.HasValue ? AdditionalJob.Value : false;
           //// ViewBag.StoreId = StoreService.StoreId;
           // // If model number is null, then try to get it from database
           
           //     var model = ProductService.GetDetails(ProductService.SessionInfo.ModelId);
           // //    modelNumber = model.ModelNumber;
           // //}
           //     Log.Database.ProductView.AddNote(model.ModelNumber);
            
             return Redirect(Url.ProcessNextStep());
        }
   
        public ActionResult GoToNewmodelDetails( string ApplianceCD, string MFR, string ModelNumber)
        {
            
                ProductService.SessionInfo.ApplianceCD = !string.IsNullOrEmpty(ApplianceCD) ? ApplianceCD.Split('-')[0].ToString().Trim() : string.Empty;
                ProductService.SessionInfo.ModelNumber = !string.IsNullOrEmpty(ModelNumber) ? ModelNumber : string.Empty;
                ProductService.SessionInfo.ItemCode = !string.IsNullOrEmpty(ModelNumber) ? ModelNumber : string.Empty;

                ProductService.SessionInfo.MFR = !string.IsNullOrEmpty(MFR) ? MFR.Split('-')[0].ToString().Trim() : string.Empty;
            if(!string.IsNullOrEmpty(MFR) )
            {
                ProductService.SessionInfo.ManufactName = MFR.Split('-')[1].ToString().Trim();
            }
        ...
        { 
            ...
            //if (modelid != null)
            //{
            //    ProductService.SessionInfo.ModelId = modelid.Value;
            //}
            if (ProductService.SessionInfo.ModelId > 0)
            {
                BookService.SessionInfo.ModelId = ProductService.SessionInfo.ModelId;
                Log.File.Info(ProductService.Msg.GenerateLogMsg("Getting product details.", "Model id = " + ProductService.SessionInfo.ModelId.ToString()));
                var details = ProductService.GetDetails(ProductService.SessionInfo.ModelId);
                Log.Database.ProductView.AddNote(details.ModelNumber);
                details.AdditioalLinks = ProductService.GetProductAdditionalServices(ProductService.SessionInfo.ModelId);
                ProductService.SessionInfo.InjectFrom(details);
                //ViewBag.AdditionalJob = ProductService.SessionInfo.AdditionalJob;
                //var ProductRepairProfiles = ProductService.GetClientModelDetails(ProductService.SessionInfo.ModelId, StoreService.StoreId);
            ...
            {
                Log.File.Info(ProductService.Msg.GenerateLogMsg("Adding new product details.", "Model id = " + ProductService.SessionInfo.ModelId.ToString()));
              // var details = ProductService.GetDetails(ProductService.SessionInfo.ModelId);
                Product_DetailModel model = new Product_DetailModel();
                model.ApplianceCD=ProductService.SessionInfo.ApplianceCD;

             //   model.ModelNum=;
                model.ModelNumber=ProductService.SessionInfo.ModelNumber;
                model.ItemCode=ProductService.SessionInfo.ModelNumber;
                model.MFR=ProductService.SessionInfo.MFR;
                model.ManufactName = ProductService.SessionInfo.ManufactName;
        ...
        [HttpPost]
        public ActionResult ShowDetails(Product_DetailModel model)
        {
          
            BookService.SessionInfo.InjectFrom(ProductService.SessionInfo);
           
            return Redirect(Url.ProcessNextStep());

        }

        // to start new process for adding new appliance for existing customer
        public ActionResult SearchAddNewAppinCustomer(int? customerid)
        {
            CustomerModel model = new CustomerModel();
            if (customerid.HasValue)
            {
                BookService.SessionInfo.CustomerId = customerid.Value;
                model = CustomerService.GetCustomerBasicInfo(customerid.Value);
                CustomerService.SessionInfo.Applianceaddress.InjectFrom(model);
            }
            return Redirect(Url.Process(PredefinedProcess.AddCustApl));
        }
