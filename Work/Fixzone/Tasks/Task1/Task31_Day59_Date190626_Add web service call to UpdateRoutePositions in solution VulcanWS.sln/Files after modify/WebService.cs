...
namespace Vulcan.WebService
{
    ...
    public class OnlineBooking
    {
        ...
        public int UpdateRoutePositions(List<Classes.TRoutePos> list)
        {
            OnlineBookingService.IFzOnlineBookingservice OBS = new IFzOnlineBookingservice();

            TRoutePosition[] routePositions = new TRoutePosition[list.Count];

            for (int i = 0; i < list.Count; i++)
            {
                routePositions[i] = new TRoutePosition() { ServiceID = list[i].ServiceID, RoutePos = list[i].RoutePos };
            }

            return OBS.UpdateRoutePositions(routePositions);
        }
    }
}
