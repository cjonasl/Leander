...
'Mary@fixzone.com'
 --Customer.EMAIL 
 AS MESSAGESRV_FAILEDLW_HTML_EMAIL,  
  ...
  '0800 092 9051' AS UKWPHONENUMBER,  f.footer as Footer,
   RC.RetailClientName as Brand,
  RC.Domain AS [Domain],
  RC.Domain+'/Content/img/ClientLogo.png' as Logo, 'We missed you' AS FAILEDLW
FROM [DiaryEnt]
...
WHERE --DiaryEnt.EventDate=CONVERT(DATE, GETDATE())AND 
 ((Customer.RetailClientID=1 AND 
 Customer.EMAIL IS NOT NULL) 
 --AND service.STATUSID=19 
 AND service.SUBSTATUS=1) 
 and  (SpecJobMapping.DummyJob <> 1   or SpecJobMapping.DummyJob is  null)
 and diaryent.diaryid=2355979
