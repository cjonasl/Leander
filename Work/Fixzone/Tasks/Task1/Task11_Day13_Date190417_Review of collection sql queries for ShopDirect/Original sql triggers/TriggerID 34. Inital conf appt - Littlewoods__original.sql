SELECT top 25
 Customer.Email 
    AS MESSAGESRV_INITCONFL_HTML_EMAIL,  
  DiaryEnt.DiaryID, 
   f.footer as Footer,
...
FROM DiaryEnt
LEFT JOIN TriggerRes ON TRIGGERID=34 AND TRIGGERFIELDLAST='DiaryID' AND TriggerRes.TriggerValue=DiaryEnt.DiaryID
...
Customer.RetailClientID=1
 AND TriggerRes.id IS NULL 
  AND custapl.policynumber like '%ESP' and
