...
'Mary@fixzone.com' 
  --Customer.EMAIL 
   AS MESSAGESRV_DEPOTVRPG_HTML_EMAIL,  
  ...
  AS TermType,
  'Your '+COALESCE(Model.[DESCription], 'item')+' has arrived' AS VeryDepotSub
FROM DiaryEnt
...
(Customer.RetailClientID=2 AND Customer.EMAIL IS NOT NULL) AND POP_Apl.MONITORFG='T' AND Enginrs.DumpDiary=0 
AND  (Custapl.POLICYNUMBER LIKE '%RPG' OR Custapl.POLICYNUMBER LIKE '%MPI')  ) and  (SpecJobMapping.DummyJob <> 1   or SpecJobMapping.DummyJob is  null)
 or Diaryent.DiaryID in (2844878,2450785)
