...
--'Mary@fixzone.com'
Customer.Email 
 AS MESSAGESRV_REMINDV_HTML_EMAIL,  
...
FROM DiaryEnt
LEFT JOIN TriggerRes ON TRIGGERID=11 AND TRIGGERFIELDLAST='DiaryID' AND TriggerRes.TriggerValue=DiaryEnt.DiaryID
...
WHERE (DiaryEnt.EventDate=CONVERT(DATE, GETDATE()+1)  AND
 Custapl.POLICYNUMBER like '%ESP'
AND(Customer.RetailClientID=2 AND Customer.EMAIL IS NOT NULL) AND COALESCE(POP_Apl.MONITORFG,'F')='F' AND Enginrs.DumpDiary=0) 
