...
namespace CAST.Repositories
{
    public class UserRepository
    {
        ...
        public User_AccountInfoModel GetUserDetailsForUpdate(string userId)
        {
            var userDetails = _dataContext.Connection
                 .Query<User_AccountInfoModel>("RetrieveUserData", new { UserID = userId }, commandType: CommandType.StoredProcedure)
                 .FirstOrDefault();
            ...
            if (userDetails != null)
            {
                ...
                {
                    userDetails.Day = userDetails.DateOfBirth.Value.Day;
                    userDetails.Month = userDetails.DateOfBirth.Value.Month;
                    userDetails.Year = userDetails.DateOfBirth.Value.Year;
                }
                else
                    userDetails.Day = userDetails.Month = userDetails.Year = 0;
            }
            return userDetails;
        }
        ...
        {
            _dataContext.Connection.Execute("DisableUser", new { userId = userId }, commandType: CommandType.StoredProcedure);
        }
    }
}