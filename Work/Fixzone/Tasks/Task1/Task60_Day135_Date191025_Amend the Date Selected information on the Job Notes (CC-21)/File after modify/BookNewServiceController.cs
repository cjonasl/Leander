...
namespace ClientConnect.Controllers
{
    ...
    public class BookNewServiceController : Controller
    {
        ...
        public ActionResult BookNow(string EventDate, int Engineerid, string faultdesc, string reason = "", JobType type = JobType.Defaulttype)
        {
            ...
            if (!BookService.SessionInfo.OnlinebookingFailed)
            {
                ...
                if (response.BookSuccessfully)
                {
                    ...
                    {
                        if (Settings.Default.DeploymentTarget == "Clients")
                            serviceNotes += string.Format("Client Preferred Date: {0}, 1st Date Available: {1}, Client Selected Date: {2}", model.VisitDate.ToString("yy-MM-dd"), ((DateTime)Session["FirstDayOffered"]).ToString("yy-MM-dd"), model.VisitDate.ToString("yy-MM-dd")); //CC-21
                        else
        ...
        {        
            ...
            if (Settings.Default.DeploymentTarget == "Clients"  && appointment.PreferredVisitDate == null)
            {
                ViewBag.ShowView_RequestDate = false;
